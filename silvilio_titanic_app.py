...
st.set_page_config(page_title="TITANIC | SILVILIO",
        layout="centered",
        page_icon="🛳",
        )
...
# ------------------ PREPROCESAMIENTO Y LIMPIEZA DE NUESTRO DATA SET -----------------------------------------------
...
#Cambiar nombre de los valores de la columna Embarked para mejor presentación.
df["Embarked"] = df["Embarked"].replace({"S": "Southampton", "C":"Cherbourg", "Q":"Queenstown"})

#Para una de las gráficas posteriores, creamos esta nueva columna con la edad por tramos.
# Crear bins de edad
bins = [0, 10, 20, 30, 40, 50, 60, 70, 81]
# Añadimos una columna nueva, rango_edad
df["rango_edad"] = pd.cut(df["Age"], bins).astype(str)

# Para otra de las gráficas, añadimos al dataframe una columna nueva, viajando_solos.
# Usamos una función que fila a fila añadirá un 1, si esas dos columnas son 0, si no añadirá un 0.
...
st.image("Imágenes/título_2.png")
...
if menu =="Introducción":
    ...
    Cierra los ojos e imagínate en el año 1912. Te acercas a las costas de Terranova mientras escuchas el
    sonido de las olas golpeando el casco del barco. Respira el aire helado de la noche atlántica.
    ¡Genial! Estás a bordo del Titanic.
    
    Pero en esta ocasión nos embarcamos en los datos que forman su historia.
    Este proyecto se centrará en el análisis de datos del trágico viaje. Exploraremos el dataset para descubrir
    información relevante de los pasajeros, así como los factores que influyeron en su supervivencia.
    

    """, unsafe_allow_html=True)
    st.image("http://c.files.bbci.co.uk/16D1/production/_100814850_gettyimages-877330410.jpg")  #imagen Titanic


    #Título primer párrafo
    st.markdown("<h3 style='text-align: left; color: #FFFFFF;'>Breve historia del Titanic</h3>" ,unsafe_allow_html=True)

    st.markdown(
                             """
    <div style="border: 0px solid #ff4b4b; padding: 8px; background-color: rgba(254, 143, 143, 0); color: #FFFFFF;">   
    El Titanic fue un transatlántico construido por la White Star Line y lanzado en 1912. Era considerado como
    el "barco más seguro jamás construido" debido a sus características de seguridad innovadoras.
    El 14 de abril de 1912, el Titanic chocó con un iceberg durante su viaje inaugural de Southampton,
    Reino Unido a Nueva York, y se hundió. <br>
    <br>

    Muchas vidas se perdieron debido a la falta de balsas salvavidas suficientes a bordo.
    El desastre del Titanic ha sido ampliamente considerado como uno de los peores accidentes
    marítimos de la historia y ha sido objeto de innumerables libros, películas y estudios.
    ...
    st.markdown("<h3 style='text-align: left; color: #FFFFFF;'>Objetivos del análisis</h3>" ,unsafe_allow_html=True)

    #Segundo párrafo
    st.markdown(
                             """
    El objetivo principal de este análisis será responder a diversas preguntas relacionadas con la supervivencia
    de los pasajeros, tales como: ¿qué factores influenciaron en la tasa de supervivencia de los pasajeros?
    ¿Cuántos pasajeros había según sus clases? 
    ¿Hubo diferencias significativas entre la tasa de supervivencia de los hombres y las mujeres?

    A partir de los resultados obtenidos, realizaremos una predicción de la supervivencia de los pasajeros
    basada en los datos que disponemos. Utilizaremos un modelo de regresión logística para estimar las
    probabilidades de supervivencia y clasificar a los pasajeros en dos grupos: sobrevivientes y no sobrevivientes.

    Esperamos que los resultados de este análisis ayuden a entender mejor los factores que influenciaron en la
    tasa de supervivencia de los pasajeros del Titanic y contribuyan a la preservación de su historia.   
    <br>
    <br>
    <br>
    <br>
    ...
    mi_sidebar= False
    if(st.button("¿Quieres conocer antes algunos detalles del hundimiento? 🚢" )):
        mi_sidebar=True
        if mi_sidebar:
            with st.sidebar:
                st.markdown("""
                ...
                st.header(":red[Datos reales en el momento del hundimiento del Titanic]")
                col1, col2 = st.columns(2)
                col1.metric("Temperatura ambiental", "4.1 °C")
                col2.metric("Temperatura del océano", "7.3 °C")

            

                # Sacamos la variable de la fecha del hundimiento y la de la fecha actual y la restamos.
                fecha_hundimiento = datetime(1912, 4, 15)
                now = datetime.now()                      # Gracias a datetime va haciendo la operación siempre con la fecha actual.
                tiempo_transcurrido = relativedelta(now, fecha_hundimiento)
                
                #Columnas representadas en el sidebar del tiempo transcurrido.
                st.header(":red[Tiempo transcurrido desde el hundimiento del Titanic el 15 de abril de 1912:]")
